...
//        ThreadPoolExecutor threadPoolExecutor = new ThreadPoolExecutor();
        ...
        ArrayBlockingQueue<Runnable> workQueue = new ArrayBlockingQueue<>(corePoolSize + 10);

        RejectedExecutionHandler handler = new ThreadPoolExecutor.CallerRunsPolicy();
        ThreadPoolExecutor threadPoolExecutor = new ThreadPoolExecutor(corePoolSize, maxPoolSize, keepAlive, TimeUnit.MILLISECONDS, workQueue, handler);

        for (String code : listSymbol) {
            Runnable runnable = new CheckCodeRunable(listCode, code, threshHold, volDefault, dayLast);
